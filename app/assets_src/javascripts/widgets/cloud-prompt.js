 ...
 *
 * Usage:
 *		modalAlert( message, [title, callback] )
 *		modalConfirm( message, [title, callback] )
 *		modalPrompt( message, [value, title, callback] )
 *
 * History:
 *
...
(function ($) {
	...
	$.alerts = {
		...
		_show: function(title, msg, value, type, callback) {
			...
			if ($.alerts.dialogClass)
				$("#alert-container").addClass($.alerts.dialogClass);
			...
			case 'prompt':
				$("#alert-message")
					.append('<br /><input type="text" size="30" id="alert-prompt" />')
					.after('<div id="alert-panel"><input type="button" value="'+ $.alerts.okButton +'" id="alert-ok" class="btn-primary" /> <input type="button" value="'+ $.alerts.cancelButton +'" id="alert-cancel" class="btn" /></div>');
	...
	$.modalPrompt = function (message, value, title, callback) {
